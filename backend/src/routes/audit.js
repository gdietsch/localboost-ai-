...
router.post('/', async (req, res) => {
  try {
    ...
    let businesses = query(
      `SELECT id FROM businesses WHERE email = '${email.replace(/'/g, "''")}' AND name = '${name.replace(/'/g, "''")}' LIMIT 1`
    );
    ...
    if (businesses && businesses.length > 0) {
      ...
      const insertResult = query(
        `INSERT INTO businesses (name, website, category, email) VALUES ('${name.replace(/'/g, "''")}', '${website.replace(/'/g, "''")}', '${category.replace(/'/g, "''")}', '${email.replace(/'/g, "''")}') RETURNING id`
      );
      businessId = insertResult[0]?.id;
    }

    // Create audit
    const auditResult = query(
      `INSERT INTO audits (business_id, status) VALUES (${businessId}, 'pending') RETURNING id`
    );
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Get content items
    const content = query(
      `SELECT id, type, title, body, status FROM content_items WHERE audit_id = '${id.replace(/'/g, "''")}' ORDER BY type, id`
    );
...
router.post('/:id/generate', async (req, res) => {
  try {
    ...
    query(`UPDATE audits SET status = 'generating' WHERE id = '${id.replace(/'/g, "''")}'`);
    ...
    // Generate review reply templates
    const reviews = [
    ...
    const insertItem = (type, title, body) => {
      ...
      query(`INSERT INTO content_items (audit_id, type, title, body, status) VALUES ('${id.replace(/'/g, "''")}', '${type}', '${safeTitle}', '${safeBody}', 'draft')`);
    };
    ...
    query(`UPDATE audits SET status = 'complete' WHERE id = '${id.replace(/'/g, "''")}'`);
...
router.patch('/content/:id/status', async (req, res) => {
  try {
    ...
    query(`UPDATE content_items SET status = '${status.replace(/'/g, "''")}' WHERE id = '${id.replace(/'/g, "''")}'`);
