...
router.post('/create-checkout-session', async (req, res) => {
  try {
    ...
    let businesses = query(`SELECT id FROM businesses WHERE email = ${safe(email)} AND name = ${safe(name)} LIMIT 1`);

    let businessId;
    if (businesses && businesses.length > 0) {
      businessId = businesses[0].id;
    } else {
      query(`INSERT INTO businesses (name, website, category, email) VALUES (${safe(name)}, ${safe(website)}, ${safe(category)}, ${safe(email)})`);
      const newBiz = query(`SELECT id FROM businesses WHERE email = ${safe(email)} AND name = ${safe(name)} ORDER BY created_at DESC LIMIT 1`);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { business_id: businessId, type: isSubscription ? 'subscription' : 'audit' },
    });

    query(`INSERT INTO payments (business_id, email, amount, currency, stripe_session_id, status, type) VALUES (${safe(businessId)}, ${safe(email)}, ${amount}, 'usd', ${safe(session.id)}, 'pending', ${safe(isSubscription ? 'subscription' : 'audit')})`);
