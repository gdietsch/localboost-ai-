...
router.post('/create-checkout-session', async (req, res) => {
  try {
    ...
    const businesses = await query(`SELECT id FROM businesses WHERE email = ${safe(email)} AND name = ${safe(name)} LIMIT 1`);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { business_id: String(businessId), type: isSubscription ? 'subscription' : 'audit' },
    });

    await execute(`INSERT INTO payments (business_id, email, amount, currency, stripe_session_id, status, type) VALUES (${safe(businessId)}, ${safe(email)}, ${amount}, 'usd', ${safe(session.id)}, 'pending', ${safe(isSubscription ? 'subscription' : 'audit')})`);
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (businessId) {
        ...
        await execute(`UPDATE payments SET status = 'completed' WHERE stripe_session_id = ${safe(session.id)}`);
