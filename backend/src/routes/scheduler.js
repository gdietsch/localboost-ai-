/**
 * Scheduler route for weekly content generation & approval (ESM - local backend).
 */
import { Router } from 'express';
import { query } from '../models/db.js';
import { sendApprovalEmail } from '../services/email.js';
import { autoPost } from '../services/poster.js';
...
router.post('/weekly-content', async (req, res) => {
  try {
    ...
    for (const biz of businesses) {
      try {
        query(`INSERT INTO audits (business_id, status) VALUES (${safe(biz.id)}, 'generating')`);
        const auditResult = query(`SELECT id FROM audits WHERE business_id = ${safe(biz.id)} ORDER BY created_at DESC LIMIT 1`);
        ...
        for (const item of items) {
          query(`INSERT INTO content_items (audit_id, type, title, body, status) VALUES (${safe(auditId)}, ${safe(item.type)}, ${safe(item.title)}, ${safe(item.body)}, 'draft')`);
        }

        query(`UPDATE audits SET status = 'complete' WHERE id = ${safe(auditId)}`);
        totalGenerated++;

        const contentItems = query(`SELECT id, type, title, body, status FROM content_items WHERE audit_id = ${safe(auditId)}`);
    ...
    res.json({ message: 'Weekly content generation complete', totalGenerated, emailsSent });
  } catch (err) {
...
router.post('/approve-content/:contentId', async (req, res) => {
  try {
    ...
    const items = query(`SELECT id, type, title, body, status FROM content_items WHERE id = ${safe(contentId)}`);
    if (!items || items.length === 0) return res.status(404).json({ error: 'Content not found' });

    const item = items[0];
    if (item.status !== 'draft') return res.json({ message: `Content already ${item.status}`, contentId });

    query(`UPDATE content_items SET status = 'approved' WHERE id = ${safe(contentId)}`);
    try { await autoPost(item); } catch (e) { console.error('Auto-post error:', e.message); }
...
router.post('/approve-all/:auditId', async (req, res) => {
  try {
    ...
    const items = query(`SELECT id, type, title, body, status FROM content_items WHERE audit_id = ${safe(auditId)} AND status = 'draft'`);
    if (!items || items.length === 0) return res.json({ message: 'No draft content', approved: 0 });

    let approved = 0;
    for (const item of items) {
      query(`UPDATE content_items SET status = 'approved' WHERE id = ${safe(item.id)}`);
      try { await autoPost(item); } catch (e) { /* ignore */ }
      approved++;
    }
